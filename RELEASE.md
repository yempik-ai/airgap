...
| 4 | `./bin/claude-local.sh -p 'reply AIRGAP OK'` from a project folder, then one probe per shipped adapter: `./bin/run.sh --probe claude-code` and `./bin/run.sh --probe codex` | `AIRGAP OK` from all three, the server log shows the requests (`thinking=` and the prompt token count), and both probe lines are kept verbatim for the notes | anything else; a shipped adapter with no probe transcript from this tag |
...
```
commit · chip / RAM / macOS · mlx-serve · Claude Code / Codex CLI · build · CTX_SIZE · non-default settings · doctor: N PASS / N WARN · bench: decode X tok/s, prefill Y tok/s at N tokens, peak Z GB · e2e: AIRGAP OK · probe claude-code: T s, N prompt tokens · probe codex: T s, N prompt tokens
```

Example, assembled from the figures already recorded for `main` on 2026-08-18
(several sittings, not one; not a tag). The two probe figures are the warm ones
MEASURED on `phase-1/harness-adapters`, same machine and date, because
`run.sh --probe` does not exist at `9b0d355`:

```
9b0d355 · M3 Max / 36 GB / macOS 26.5.2 · mlx-serve 26.8.8 · Claude Code 2.1.234 / Codex CLI 0.147.0 · 9b-4bit · 65536 · none · doctor: 29 PASS (with server) · bench: 36.7 tok/s after 41 tokens, 15.6 after 16,377; prefill 374 tok/s at 16,377; peak 5.63 GB · e2e: AIRGAP OK · probe claude-code: 4.8 s, 20,718 prompt tokens · probe codex: 3.4 s, 9,336 prompt tokens
```
